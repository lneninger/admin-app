...
export class LazyLoaderService {
  ...
  async load(name: string, container: ViewContainerRef, data?: any, dataToken: any = LAZY_COMPONENT_METADATA, defaultName: string = null, loading?: (loading: LazyLoadingStatus) => void) {
    ...
    if (container.length === 0) {
      const tempModule = await this.downloadResource(name, defaultName, loading);
      if (!tempModule) {
        return;
      }
    ...
    if (views) {
      views.forEach(async viewDirective => {
        ...
        if (
          ...
          await this.load(viewDirective.appLazy, viewDirective.viewContainerRef, viewDirective.metadata, LAZY_COMPONENT_METADATA, viewDirective.default, viewDirective.setLoading);
        }
      });
    }
  }
