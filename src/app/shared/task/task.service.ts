...
import { PreviewFeatureService } from './../../shared/preview-feature/preview-feature.service';
import {
...
export class TaskService extends NgxsDataRepository<ICommandsWrapper> implements ICommandService {
    ...
    async activate() {
        await this.previewFeatureService.initialized$.pipe(filter(_ => !!_), first()).toPromise();
        if (this.previewFeatureService.isPreview('preview-membersession')) {
